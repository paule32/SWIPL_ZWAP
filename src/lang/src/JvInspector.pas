...
var
  comp_btn_color: TColor;
  ActiveDesignerControl : TControl;
  ActiveSelectedInspectorItem : TJvCustomInspectorItem;
...
procedure TJvCustomInspectorItem.EditMouseDown(
  Sender: TObject;
  Button: TMouseButton;
  Shift:  TShiftState;
  X, Y:   Integer);
var
  tc : TJvCustomInspectorItem;
//  colorDialog : TJvColorDialog;
  key: char;
  cn : String ;
begin
  //showmessage(Sender.ClassName);
  if (Button = mbLeft) then //and (ssDouble in Shift) and (iifValueList in Flags) then
  begin
    if Sender.ClassName = 'TJvInspectorEdit' then
    begin
      tc := TJvInspector((Sender as TJvInspectorEdit).Parent).Selected;
      ActiveSelectedInspectorItem := tc;
      if tc.Parent.DisplayName = 'Appearence' then
      begin
        if tc.DisplayName = 'Color' then
        begin
        (*
          try
            colorDialog := TJvColorDialog.Create(TJvInspector((Sender as TJvInspectorEdit).Parent));
            if colorDialog.Execute then
            begin
              key := #13;
              tc.GetEditCtrl.Text := ColorToString(colorDialog.Color);
              tc.EditKeyPress(tc.GetEditCtrl,key);
              comp_btn_color      := colorDialog.Color;
              cn := ActiveDesignerControl.ClassName;
              if cn = 'TJvImgBtn' then
              begin
                TJvImgBtn(ActiveDesignerControl).Color := comp_btn_color;
              end;
            end;
          finally
            colorDialog.Free;
